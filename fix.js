...
// 1. Add inline price CSS after .merch-price
html = html.replace(
...
// 2. T-shirt: inline price + no unit price + horizontal qty
const tshirtOld = `<div class="merch-info">
...
const tshirtNew = `<div class="merch-info">
...
// 5. Make openCart/closeCart empty
html = html.replace(
...
// 7. Make renderCart empty stub
html = html.replace(
  /\/\/ ── Render Cart ─────────────────────────────────────────────────\n  function renderCart\(\) \{/,
  `function renderCart() { }`
);

// 8. Add null check for merch-unit in updateTotal
html = html.replace(
  /if \(right\) right\.querySelector\('\.merch-unit'\)\.style\.display = '';/g,
  `if (right) { const unit = right.querySelector('.merch-unit'); if (unit) unit.style.display = ''; }`
);
html = html.replace(
  /if \(right\) right\.querySelector\('\.merch-unit'\)\.style\.display = 'none';/g,
